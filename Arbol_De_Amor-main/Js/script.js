...
function showDedicationText() { //seguidores
  ...
  if (!text) {
    text = `Para la futura campeona:\n\nquiero decirte esto te conoci hace pocos dias y aun asi me contaste lo que te paso. se que es feo pasar por eso y por eso te digo esto\n\nanimo niña cuchilla no te he visto jugar pero se que debes ser muy buena aqui tienes tu primer fan Osea yo\n\nyo comfio en ti lo vas a lograr echale ganas.`;  } else {
...
function showSignature() {
  ...
  signature.textContent = firma ? decodeURIComponent(firma) : "de mi para spider woman";
...
function showCountdown() {
  ...
  let startDate = startParam ? new Date(startParam + 'T00:00:00') : new Date('2025-08-17T00:00:00'); 
  let eventDate = eventParam ? new Date(eventParam + 'T00:00:00') : new Date('T00:00:00');
  ...
  function update() {
    ...
    container.innerHTML =
      `Campeona: <b>${days}</b> días<br>` +
      `Profesional: <b>${eventDays}d ${eventHours}h ${eventMinutes}m ${eventSeconds}s</b>`;
